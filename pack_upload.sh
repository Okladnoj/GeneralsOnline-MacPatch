...
#
# Trees: GO_Mac_Patch/, GO_Mac_Mod_Contra007|008|009|ContraX/
# ContraX → GO_Mac_Mod_ContraX.{1,2,3}.zip (see contrax_parts.sh).
#
# Usage:
#   ./pack_upload.sh                  # all five, pack + upload
#   ./pack_upload.sh --no-upload      # zip only
#   ./pack_upload.sh Patch Contra007
#   ./pack_upload.sh ContraX --no-upload
set -euo pipefail
...
for t in "${TARGETS[@]}"; do
  case "$t" in
    ...
    Contra007|Contra008|Contra009) pack_dir "GO_Mac_Mod_$t" "GO_Mac_Mod_$t.zip" 1 ;;
